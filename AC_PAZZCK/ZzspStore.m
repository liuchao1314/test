...
#import "ZzspStore.h"
#import "User.h"
static ZzspStore* zzspStore=nil;
@implementation ZzspStore
@synthesize flowList;
+(ZzspStore*) zzspStore{
    NSLog(@"%@  --  %@",NSStringFromClass([self class]),NSStringFromSelector(_cmd));
    if (!zzspStore) {
        zzspStore=[[super allocWithZone:nil] init];
    }
    return zzspStore;
}

+(id) allocWithZone:(NSZone *)zone{
    return [self zzspStore];
}
...
- (NSString *)archivePath
{
    ...
    NSString* fileName=[[[[UserInfo sharedUser] user] carNum] stringByAppendingString:@"-zzsp.data"];
    NSLog(@"%@",fileName);
